...
$(document).ready(function () {
  ...
  //Changes username to "Your username"
    $('#Admin-Home #account-menu-dropdown span').html('Your username').attr('style', 'padding-right: 10px');

  //Removes the "Search other Next catalogs" button from the footer
    $('#Admin-Home #footer-custom-text').remove();

  //Hides the state library link
    $('#Admin-Home a[href="/NextLibraries"], a[href="https://library.ks.gov/home"]').hide();

  //Removes the logo and adds Primary/Secondary/Tertiary/Quaternary text block where the logo used to be
    $('#Admin-Home #header-logo').parent().parent().html(
  ...
  //Adds a border to the permissions panels in the Administration home
    $('#Admin-Home .grid-col--2 .adminSection.grid-item .adminPanel').css({ 'border': '10px solid #73e55e', 'border-radius': '12px' });
